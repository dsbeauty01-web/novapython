...
from livekit.plugins import google as google_plugin  # v222: Gemini brain
...
async def entrypoint(ctx: JobContext):
    ...
    # v222: GEMINI brain via the livekit google plugin (uses GOOGLE_API_KEY env).
    # gemini-2.5-flash-lite = fast, no "thinking" latency → good for real-time voice.
    # STT stays OpenAI on the mic track (browser also sends user-said); TTS stays
    # ElevenLabs loora; avatar stays Runway. Brain is the A/B variable vs v113.
    llm_instance = google_plugin.LLM(
        model=os.getenv("NOVA_GEMINI_MODEL", "gemini-2.5-flash-lite"),
        temperature=float(os.getenv("NOVA_TEMPERATURE", "0.85")),
    )
    logger.info(f"[nova-v222] brain = Gemini {os.getenv('NOVA_GEMINI_MODEL', 'gemini-2.5-flash-lite')}")
