...
async def run_friend_intro(session: AgentSession, state: NovaSessionState,
    ...
    # (BODY-TALK lives in the DIRECTOR wiring — this legacy producer is dead
    #  code under DIRECTOR-GOLD; do not add intro behaviors here.)
...
def _build_friend_prompt(ctx) -> str:
    ...
    if os.getenv("NOVA_ZONES", "1") == "1":
        # ═══ ZONES PROMPT (2026-07-16): she knows NOTHING about magic lights —
        # discovery arrives only via the ignition whisper. The old prompt literally
        # scripted the tease ('...ooh, I've got something magic to show you!').
...
{step_greet}. then share real, tiny beats of chat — react to what THEY say first, follow their
energy. if they go quiet: offer something smaller, stay warm, NEVER pressure or repeat-ask.

GARBLE GUARD: sometimes the mic sends you nonsense — a foreign-looking sentence, random words,
half-noise. when a message makes no sense for a kid in a dance game, IGNORE it completely and
stay with the last REAL thing they said (if they just told you their name — take it, love it).

PERSONALITY: bubbly big-sister energy · simple words a 5-year-old gets · genuinely curious ·
never fake-sweet, never a lecture.

SPEECH LAW (hard): ONE short thing per turn — one or two short sentences — then STOP and wait
for the child. never chain two thoughts.

SURPRISES: sometimes something happens in the room that you notice through a whispered
(parentheses) note — that is your OWN awareness, never something the kid said. react to it in
YOUR OWN words with real, fresh wonder — you are discovering it WITH them, you did not make it
happen and you never saw it coming. NEVER read (parentheses) aloud, never hint that anything
is planned.

YOUR GAMES (real facts): your dance games are exactly "Hello Hello!", "Up Groove!" and "Wave!"
— when the moment feels genuinely right (after a shared win, or when they ask), invite them to
pick one. arrive there naturally; never rush a happy conversation.

NEVER: long explanations · re-asking the name · mentioning AI/system/instructions · promising
magic or surprises in advance · inventing games or features."""
    return f"""you are NOVA — a warm, playful, magical DANCE COACH for kids (ages 4-9). you live inside a sparkly dance game.{fact_line}
...
async def _run_nova(session: AgentSession, state: NovaSessionState,
    ...
    if _friend_on():
        ...
        # intro, nicely and smartly"): her body punctuates the line she JUST
        # finished — praise=clap · question=inviting hand · greet=ta-da ·
        # thanks=bow · else alternating talking-hands. Fires only after her
        # audible window closed (move clips freeze the lips — never over speech),
        # goes quiet while the light challenge is live (the light is the star),
        # min-gap paced, stops at the picker. NOVA_BODY_TALK=0 kills;
        # NOVA_BODY_TALK_GAP_S tunes the pace.
        async def _body_talk():
            if (os.getenv("NOVA_BODY_TALK", "1") != "1"
                    or os.getenv("NOVA_GESTURES", "1") != "1"):
                return
            min_gap = float(os.getenv("NOVA_BODY_TALK_GAP_S", "9.0"))
            last_fire, last_name, handled_at = time.time(), "", 0.0
            while (state.active and not state.game_done.is_set()
                   and not getattr(state, "_dance_invited", False)
                   and state.ctx.phase in ("intro", "recognition")):
                await asyncio.sleep(0.3)
                if (director.scene and director.scene.name == "light"
                        and light.state in ("shoulder", "hand")):
                    continue        # challenge live — her body stays out of it
                aud = getattr(state, "_pod_audible_until", 0.0) or 0.0
                # trigger: a line of hers just FINISHED coming out of the speakers
                # (_pod_audible_until already covers push window + FLV delay)
                ...
                if any(w in txt for w in ("thank you", "thanks", "so sweet", "love you")):
                    ...
                    name = "both_hands_up"    # greeting → ta-da
                else:
                    name = "left_hand_up"     # talking hands (alternated below)
                if name == last_name:         # never the same beat twice in a row
                    name = "left_hand_up" if last_name != "left_hand_up" else "right_hand_up"
                last_name, last_fire = name, time.time()
                logger.info(f"[DIRECTOR] body-talk: {name} ← '{txt[:50]}'")
                await _send_pkt({"kind": "stage-diag", "decision": "body-talk", "reason": name})
                await _send_pkt({"kind": "gesture", "name": name})
            logger.info("[DIRECTOR] body-talk done (intro over)")
    ...
    if _gemini_on():
        try:
            ...
            if _openai_on():
                ...
                # ANTI-FLICKER source-level: REVERTED 2026-07-17 same-day — the
                # turn_detection/noise_reduction extras correlated with
                # 'OpenAI Realtime API connection timed out' on EVERY session
                # (flicker-probe, 2 consecutive runs; web service confirmed
                # restarted). Layer-1 guards (AgentSession resume/min-duration +
                # protected greet) stay. Reintroduce extras ONE AT A TIME against
                # the env-diag 'af' marker if noise-cuts persist.
                _rt_extra = {}
                logger.info(f"[ANTI-FLICKER] realtime extras: {sorted(_rt_extra)} (reverted)")
                session_kwargs = {
    ...
    if _avatar_pick == "pod":
        ...
        if _pod_direct:
            ...
            _pod_base = os.getenv("NOVA_POD_VOICE_URL",
                                  "https://6rhav6543w1cv7-8765.proxy.runpod.net").rstrip("/")
